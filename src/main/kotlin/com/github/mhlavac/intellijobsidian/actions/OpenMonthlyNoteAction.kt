...
class OpenMonthlyNoteAction : AnAction() {
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        val config = vault.config.monthly
        if (config == null || !config.enabled) return

        val today = LocalDate.now()
        val format = config.getEffectiveFormat(PeriodType.MONTHLY.defaultFormat)
        val filename = PeriodicNotesUtil.formatMonthly(today, format)

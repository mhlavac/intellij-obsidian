...
class OpenQuarterlyNoteAction : AnAction() {
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        val config = vault.config.quarterly
        if (config == null || !config.enabled) return

        val today = LocalDate.now()
        val format = config.getEffectiveFormat(PeriodType.QUARTERLY.defaultFormat)
        val filename = PeriodicNotesUtil.formatQuarterly(today, format)
        ...
        if (noteFile != null) {
            ...
            NotificationGroupManager.getInstance()
                ...
                .createNotification(
                    "Failed to create quarterly note",

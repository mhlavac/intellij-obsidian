...
class OpenWeeklyNoteAction : AnAction() {
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        val config = vault.config.weekly
        if (config == null || !config.enabled) return

        val today = LocalDate.now()
        val format = config.getEffectiveFormat(PeriodType.WEEKLY.defaultFormat)
        val filename = PeriodicNotesUtil.formatWeekly(today, format)
